...
實作於 2026-05-05 ~ 2026-05-07 完成，主分支已合併並上線。本節記錄 reviewer 對最終 commit 的靜態驗證結果（live HTTP probe 未在此 session 執行，留給使用者自行 `curl -I` 確認）。

### Commit 軌跡

```
1e7f806 phase7: update README + plan to reflect post-migration state
d3c00ae phase6: GH Pages → redirect-only shell (Astro full build stays on CF Pages)
54eeac8 pulse: add 7 items 2026-05-04..05-06
54bb5ee merge: Phase 3+4 — Astro config to root, GA4, sitemap, security headers
8e8cf83 migrate(cf-pages): add Permissions-Policy header
65b516b migrate(cf-pages): Phase 3+4 — Astro config, GA4, sitemap, security headers
0c15e74 docs(migration-plan): switch to Option B — DNS stays at Gandi
d325518 docs(dns-audit): chatbot.tw DNS audit before Cloudflare migration
8e4d120 docs(migration-plan): respond to reviewer findings, all adopted
```
...
| 審查項                            | 實作 file:line                                                                                          | 驗證                     |
| --------------------------------- | ------------------------------------------------------------------------------------------------------- | ------------------------ |
| Sitemap 不存在 (CRITICAL)         | `site/package.json:16` `@astrojs/sitemap ^3.7.2`；`site/astro.config.mjs:3,16-28` 整合設定              | 安裝 + i18n locales 對應 |
| Custom domain 切換時序 (CRITICAL) | `site/astro.config.mjs:7` `site: "https://claude-pulse.chatbot.tw"`；無 `base`，已 merge main           | 已合併、生效             |
| Node 版本不符 (HIGH)              | `site/package.json:5-7` `engines.node: >=22.12.0`；`.github/workflows/deploy.yml:27` `node-version: 22` | 兩處對齊                 |
| feed.xml.js 硬編碼 (HIGH)         | `site/src/pages/feed.xml.js:16` `link: \`${context.site}#${...}\``                                      | `claude-pulse/` 已移除   |
| GA env var silent ship (HIGH)     | `site/src/layouts/Base.astro:39, 44` 直接 hardcode `G-R69N8EZQE7`                                       | 採更激進方案，無 env var |
| feed.xml 漏 redirect (HIGH)       | `site/scripts/make-redirects.mjs:73-97` 產生 valid RSS + 搬家通知 item，GUID `migration-notice`         | feed 路徑不再 404        |
| Change of Address 要 301 (HIGH)   | 計畫 6.3 註記 90 天觀察期 + canonical fallback                                                          | 文字層處理               |
| DNS audit MX/SPF/DKIM/CAA (HIGH)  | `docs/dns-audit/`（commit `d325518`）                                                                   | 已執行並留檔             |
| Layout.astro → Base.astro (LOW)   | `site/src/layouts/Base.astro` 已加 GA4                                                                  | 路徑修正                 |
| Security headers 補充             | `site/public/_headers` 完整含 HSTS / CSP / X-Frame-Options / Permissions-Policy / Referrer-Policy       | 7 項 headers 全到位      |

### 實作品質高於原計畫的部分

1. **`make-redirects.mjs` 區分 `redirectHTML` vs `notFoundHTML`** — 原審查只要求覆蓋 `feed.xml`，實作做了更細的語義區分：known 200 paths 用 canonical（讓 Google 轉移索引）、404 用 noindex（避免 404 頁本身被索引）。canonical 與 noindex 不混用，避免互相抵銷。
2. **CF Pages vs GH Actions 雙路 deploy 清晰分離** — `make-redirects.mjs` 只在 GH Actions 跑（`deploy.yml:37-39`），CF Pages 拉 git 後 `npm run build` 即停。Comment 寫明此設計，避免未來 reviewer 誤解。
3. **`feed.xml` 搬家通知 GUID 用 `migration-notice`** — 與歷史 item 的 GUID 格式不衝突，訂閱者不會看到重複歷史。

### 已驗證但仍建議使用者自跑的 live check

```bash
# 從外部 probe 服務驗證（避免本機網路問題誤判）
curl -I https://claude-pulse.chatbot.tw/
curl -I https://claude-pulse.chatbot.tw/feed.xml
curl -I https://claude-pulse.chatbot.tw/sitemap-index.xml
curl -I https://claude-pulse.chatbot.tw/zh-TW/
curl -I https://clementtang.github.io/claude-pulse/        # 應 200 + meta refresh
curl -I https://clementtang.github.io/claude-pulse/feed.xml # 應 200 + 搬家 RSS
```

GA4 Realtime 報表須親訪 5 個 locale 各一次確認 event 收齊（不能只看 DevTools Network 200，否則會被 hardcode typo 騙過 — 雖然此實作直接 hardcode，typo 風險低但不為零）。

### 潛在小問題（未阻塞，記錄供日後檢視）

1. **CSP `script-src 'unsafe-inline'`** — GA4 的 `gtag('config', ...)` inline call 需要，這是必要妥協。`static.cloudflareinsights.com` 也 allow，但目前未確認 CF Pages 是否實際注入 — 若無則可收緊 CSP。
2. **`astro.config.mjs:8` `trailingSlash: "always"`** — 原計畫沒提，實作新加。風險：舊書籤 `/zh-TW`（無尾斜線）meta refresh 跳到 `/zh-TW`，CF Pages 會 redirect 到 `/zh-TW/`，多一跳。可接受。
3. **`MEMORY.md` 已加 `feedback_gsc_sitemap_full_url.md`** — 實作時踩到的小坑（GSC 提交 sitemap 必須完整 URL），已記憶留存。

### 結論

實作完整對齊審查意見，且 `make-redirects.mjs` 的 canonical/noindex 區分屬於實作期間的額外巧思。**沒有發現需倒回去修正的項目**，可進入 Phase 8 觀察期：

- 30 天：GA4 Realtime + GSC indexing 進度抽查
- 90 天：舊 GH Pages 流量歸零後評估完全停用 GH Pages 部署
